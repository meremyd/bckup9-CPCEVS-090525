...
export default function VoterSSGResultsPage() {
  ...
  // ✅ FIXED: Updated renderPositionResults to properly handle both overall and department results
  const renderPositionResults = (positions, title, showDepartmentName = false, isCompleted = null) => {
    if (positions.length === 0) return null

    // Use passed isCompleted status, or fallback to election status
    const shouldShowNames = isCompleted !== null ? isCompleted : election?.status === 'completed'
        ...
        <div className="max-w-7xl mx-auto">
          ...
          {!loadingDepartment && departmentResults && (
            <div className="mb-12">
              ...
              {/* ✅ FIXED: Pass election status to rendering function */}
              {renderPositionResults(
                groupedDepartmentPositions.presidential, 
                'PRESIDENTIAL RACE', 
                true,
                departmentResults.election?.status === 'completed'
              )}
              {renderPositionResults(
