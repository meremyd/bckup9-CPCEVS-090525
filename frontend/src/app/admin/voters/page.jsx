...
import { Search, Plus, Edit, Trash2, UserX, UserCheck, Users, Loader2, AlertCircle, Upload, Download } from "lucide-react"
...
export default function VotersPage() {
  ...
  // FIXED: Use the new activate API endpoint
  const handleActivateVoter = async (voterId, voterName) => {
    const confirmed = await showConfirm(
      "Activate Voter?",
      `This will reactivate ${voterName}. They will be able to vote again.`,
      "Yes, activate!"
    )
    if (!confirmed) return
    try {
      setLoading(true)
      await votersAPI.activate(voterId)  // Use the new activate method
      await refreshData()
      showAlert("success", "Success!", "Voter has been activated successfully")
    } catch (error) {
      showAlert("error", "Error!", handleAPIError(error))
    } finally {
      setLoading(false)
  ...
  const handleDeactivateVoter = async (voterId, voterName) => {
    const confirmed = await showConfirm(
      "Deactivate Voter?",
      `This will deactivate ${voterName}. They will not be able to vote and will be moved to the inactive list.`,
      "Yes, deactivate!"
    )
    if (!confirmed) return
    try {
      ...
      await refreshData()
      showAlert("success", "Success!", "Voter has been deactivated successfully")
    } catch (error) {
      showAlert("error", "Error!", handleAPIError(error))
    } finally {
      setLoading(false)
  ...
  const handleDeleteVoter = async (voterId, voterName) => {
    const confirmed = await showConfirm(
      "Delete Voter?",
      `Are you sure you want to permanently delete ${voterName}? This action cannot be undone.`,
      "Yes, delete!"
    )
    if (!confirmed) return
    try {
      ...
      await refreshData()
      showAlert("success", "Success!", "Voter has been deleted successfully")
    } catch (error) {
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200/50">
              ...
              <tbody className="bg-white/50 divide-y divide-gray-200/50">
                {voters.length === 0 ? (
                  <tr>
                    <td colSpan="6" className="px-3 sm:px-6 py-8 text-center text-[#001f65]/60">
                    ...
                    <tr key={voter._id} className="hover:bg-[#b0c8fe]/10">
                      ...
                      <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex justify-end space-x-2">
                          {/* FIXED: Conditional rendering - Edit button only in active tab */}
                          {activeTab === 'active' ? (
                            <>
                              <button
                              ...
                              <button
                                onClick={() => handleDeactivateVoter(voter._id, `${voter.firstName} ${voter.lastName}`)}
                            ...
                            <>
                              {/* FIXED: No Edit button in inactive tab */}
                              <button
                                onClick={() => handleActivateVoter(voter._id, `${voter.firstName} ${voter.lastName}`)}
                              ...
                              <button
                                onClick={() => handleDeleteVoter(voter._id, `${voter.firstName} ${voter.lastName}`)}
            ...
            <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-xl max-w-6xl w-full max-h-[90vh] overflow-y-auto border border-white/20">
              ...
              <div className="p-6">
                <form onSubmit={handleBulkAdd} className="space-y-4">
                  {bulkForms.map((form, index) => (
                    <div key={index} className="border border-gray-200 rounded-lg p-4 relative">
                      <div className="flex justify-between items-center mb-3">
                        ...
                        {bulkForms.length > 1 && (
                          <button type="button" onClick={() => removeBulkForm(index)} className="text-red-500 hover:text-red-700">
                            ×
                          </button>
                        )}
                      </div>
                      <div className="grid grid-cols-6 gap-3">
                        <div>
                          <label className="block text-xs font-medium text-[#001f65] mb-1">School ID *</label>
                          <input
                            type="text"
                            value={form.schoolId}
                        ...
                        <div>
                          <label className="block text-xs font-medium text-[#001f65] mb-1">First Name *</label>
                          <input
                            type="text"
                        ...
                        <div>
                          <label className="block text-xs font-medium text-[#001f65] mb-1">Middle Name</label>
                          <input
                            type="text"
                        ...
                        <div>
                          <label className="block text-xs font-medium text-[#001f65] mb-1">Last Name *</label>
                          <input
                            type="text"
                            value={form.lastName}
                        ...
                        <div>
                          <label className="block text-xs font-medium text-[#001f65] mb-1">Sex</label>
                          <select
                            value={form.sex}
                        ...
                        <div>
                          <label className="block text-xs font-medium text-[#001f65] mb-1">Department *</label>
                          <select
                  ...
                  <button type="button" onClick={addBulkForm} className="w-full px-4 py-2 border-2 border-dashed border-[#001f65] text-[#001f65] rounded-lg hover:bg-[#001f65]/10 transition-colors flex items-center justify-center gap-2">
                    <Plus className="w-4 h-4" />
                    Add Another Voter
                  </button>
                  <div className="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-3 pt-4">
                    <button type="button" onClick={closeModals} className="w-full sm:w-auto px-4 py-2 text-gray-700 bg-gray-200 rounded-lg hover:bg-gray-300">
                      Cancel
                    </button>
                    <button type="submit" disabled={loading} className="w-full sm:w-auto px-4 py-2 bg-[#001f65] text-white rounded-lg hover:bg-[#003399] disabled:opacity-50">
            ...
            <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto border border-white/20">
              ...
              <div className="p-6">
                <form onSubmit={handleEditVoter} className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-[#001f65] mb-1">School ID *</label>
                    <input
                      type="text"
                      name="schoolId"
                      value={formData.schoolId}
                      ...
                      maxLength={8}
                      required
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#001f65]"
                    />
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-[#001f65] mb-1">First Name *</label>
                      <input
                        type="text"
                        name="firstName"
                        value={formData.firstName}
                    ...
                    <div>
                      <label className="block text-sm font-medium text-[#001f65] mb-1">Middle Name</label>
                      <input
                        type="text"
                        name="middleName"
                        value={formData.middleName}
                    ...
                    <div>
                      <label className="block text-sm font-medium text-[#001f65] mb-1">Last Name *</label>
                      <input
                        type="text"
                        name="lastName"
                        value={formData.lastName}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-[#001f65] mb-1">Sex</label>
                    <select
                      name="sex"
                      value={formData.sex}
                      onChange={handleInputChange}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-[#001f65] mb-1">Department *</label>
                    <select
                      name="departmentId"
                      value={formData.departmentId}
                      onChange={handleInputChange}
